...
Using the ETHERNET port is useful when connecting MachineMotion to a standard, multi-device Ethernet network. This use case becomes necessary when connecting multiple MachineMotion controllers together in multi-controller systems and when connecting MachineMotion on a LAN or WLAN.
...
> The network configuration tab is only available in machine motion controllers with firmware >1.12.0
> To access this feature, please upgrade your firmware to >1.12.0 and ensure the Python SDK version is >2.0
